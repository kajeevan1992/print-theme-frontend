...
export default function Checkout({ cart, navigate }) {
  ...
  async function handleArtworkUpload() {
    ...
    const res = await uploadArtwork(artworkState.file);
    if (res?.success || res?.id || res?.url) {
      setArtworkState((prev) => ({ ...prev, uploaded: res, error: "" }));
    } else {
      setArtworkState((prev) => ({ ...prev, error: res?.message || "Artwork upload failed. You can still upload later." }));
    }
  }
    ...
    <section className="py-6">
      <div className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-5 flex flex-wrap gap-2">
          {STEPS.map((label, i) => <StepPill key={label} label={label} index={i + 1} current={step} />)}
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
          <div className="rounded-[22px] border bg-white p-5 shadow-[0_12px_28px_rgba(0,0,0,0.03)]" style={{ borderColor: "#E2E6E8" }}>
            ...
            {step === 5 && (
              <div className="grid gap-4">
                <div className="grid gap-3 md:grid-cols-2">
                  <button onClick={() => setArtworkState((p) => ({ ...p, mode: "now" }))} className="rounded-[16px] border bg-white p-4 text-left" style={{ borderColor: artworkState.mode === "now" ? "rgb(24, 167, 208)" : "#E2E6E8", boxShadow: artworkState.mode === "now" ? "inset 0 0 0 1px rgb(24, 167, 208)" : "none" }}>
                    <div className="flex items-center gap-3">
                      <div className="grid h-10 w-10 place-items-center rounded-xl bg-[#F1FAFD]" style={{ color: "rgb(24, 167, 208)" }}><Upload className="h-5 w-5" /></div>
                      <div>
                        <div className="text-[14px] font-bold" style={{ color: "#121517" }}>Upload artwork now</div>
                        <div className="text-[12px]" style={{ color: "#667179" }}>Attach files before placing the order.</div>
                      </div>
                    </div>
                  </button>
                  <button onClick={() => setArtworkState((p) => ({ ...p, mode: "later" }))} className="rounded-[16px] border bg-white p-4 text-left" style={{ borderColor: artworkState.mode === "later" ? "rgb(24, 167, 208)" : "#E2E6E8", boxShadow: artworkState.mode === "later" ? "inset 0 0 0 1px rgb(24, 167, 208)" : "none" }}>
                    <div className="flex items-center gap-3">
                      <div className="grid h-10 w-10 place-items-center rounded-xl bg-[#F1FAFD]" style={{ color: "rgb(24, 167, 208)" }}><FileText className="h-5 w-5" /></div>
                      <div>
                        <div className="text-[14px] font-bold" style={{ color: "#121517" }}>Upload artwork later</div>
                        <div className="text-[12px]" style={{ color: "#667179" }}>Place the order first and send files after.</div>
                      </div>
                    </div>
                  </button>
                </div>
            ...
            {step === 7 && (
              <div className="grid gap-3">
                {["Pay now", "Invoice me later", "Request proforma invoice"].map((method) => (
                  <button key={method} onClick={() => setField("paymentMethod", method)} className="flex items-center justify-between rounded-[14px] border bg-white p-4 text-left shadow-[0_6px_16px_rgba(0,0,0,0.02)]" style={{ borderColor: form.paymentMethod === method ? "rgb(24, 167, 208)" : "#E2E6E8", boxShadow: form.paymentMethod === method ? "inset 0 0 0 1px rgb(24, 167, 208)" : "0 6px 16px rgba(0,0,0,0.02)" }}>
                    <div className="flex items-center gap-3">
                      <div className="grid h-10 w-10 place-items-center rounded-xl bg-[#F1FAFD]" style={{ color: "rgb(24, 167, 208)" }}><CreditCard className="h-5 w-5" /></div>
                      <div>
                        <div className="text-[14px] font-bold" style={{ color: "#121517" }}>{method}</div>
                        <div className="text-[12px]" style={{ color: "#667179" }}>Payment-step placeholder ready for gateway integration.</div>
                      </div>
                    </div>
            ...
            {step === 8 && (
              <div className="rounded-[18px] border bg-[#F8FBFC] p-6 text-center" style={{ borderColor: "#E2E6E8" }}>
                <div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-[rgb(24,167,208)] text-white"><Check className="h-6 w-6" /></div>
                <div className="mt-4 text-[28px] font-black tracking-[-0.04em]" style={{ color: "#121517" }}>
                  {submitState?.success === false ? "Order submitted with fallback state" : "Order submitted"}
                </div>
                <p className="mx-auto mt-3 max-w-[600px] text-[13px] leading-7" style={{ color: "#667179" }}>
                  {submitState?.message || "This confirmation page is ready to connect to your real payment and order APIs."}
                </p>
                <div className="mt-5 flex flex-wrap justify-center gap-3">
                  <button onClick={() => navigate("/account")} className="rounded-full bg-[#121517] px-5 py-3 text-[12px] font-bold text-white">Go to account</button>
                  <button onClick={() => navigate("/")} className="rounded-full border px-5 py-3 text-[12px] font-bold" style={{ borderColor: "#E2E6E8", color: "#121517", backgroundColor: "white" }}>Back to storefront</button>
                </div>
              </div>
            )}
            ...
            {step < 8 && (
              <div className="mt-6 flex items-center justify-between gap-3">
                <button disabled={step === 1} onClick={() => setStep((s) => Math.max(1, s - 1))} className="rounded-full border px-5 py-3 text-[12px] font-bold disabled:opacity-50" style={{ borderColor: "#E2E6E8", color: "#121517", backgroundColor: "white" }}>
                  Back
                </button>

                {step < 7 && (
                  <button disabled={!canContinue} onClick={() => setStep((s) => s + 1)} className="rounded-full bg-[rgb(24,167,208)] px-5 py-3 text-[12px] font-bold text-white disabled:opacity-50">
                    Continue
                  </button>
                )}
          ...
          <div className="rounded-[22px] border bg-white p-5 shadow-[0_14px_30px_rgba(0,0,0,0.038)] lg:sticky lg:top-24" style={{ borderColor: "#E2E6E8", height: "fit-content" }}>
            <div className="text-[20px] font-black tracking-[-0.03em]" style={{ color: "#121517" }}>Order summary</div>
            <div className="mt-4 grid gap-3">
              {items.length === 0 ? (
                <div className="rounded-[14px] border bg-[#FBFCFD] px-4 py-4 text-[12px]" style={{ borderColor: "#E2E6E8", color: "#667179" }}>
                  Your cart is empty.
                </div>
              ) : (
                items.map((item) => (
                  <div key={item.id} className="rounded-[14px] border bg-[#FBFCFD] px-4 py-4" style={{ borderColor: "#E2E6E8" }}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-[13px] font-bold" style={{ color: "#121517" }}>{item.name}</div>
                        <div className="mt-1 text-[11px]" style={{ color: "#667179" }}>Qty {item.qty}</div>
                      </div>
                      <div className="text-[13px] font-black" style={{ color: "#121517" }}>{currency(item.price * item.qty)}</div>
                    </div>
                  </div>
                ))
              )}
            ...
            <div className="mt-5 space-y-3 text-[12px]" style={{ color: "#667179" }}>
              <div className="flex justify-between"><span>Subtotal</span><span>{currency(subtotal)}</span></div>
              <div className="flex justify-between"><span>VAT</span><span>{currency(vat)}</span></div>
              <div className="flex justify-between"><span>Delivery</span><span>{currency(deliveryFee)}</span></div>
            </div>
            <div className="mt-4 border-t pt-4" style={{ borderColor: "#E2E6E8" }}>
              <div className="flex justify-between text-[16px] font-black" style={{ color: "#121517" }}>
                <span>Total</span>
